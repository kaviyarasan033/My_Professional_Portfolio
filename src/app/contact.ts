    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" #contactSection>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div class="contact-item opacity-0 space-y-8">
          <div class="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-shadow duration-300">
            ...
            <div class="mt-10 pt-8 border-t border-slate-100 dark:border-slate-800">
              ...
              <div class="flex gap-4">
                <a href="https://github.com/kaviyarasan033" target="_blank" class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-primary-green hover:text-white hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-md">
                ...
                <a href="https://www.linkedin.com/in/kaviyarasan-m-172542299" target="_blank" class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-[#0A66C2] hover:text-white hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-md">
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path fill-rule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clip-rule="evenodd" />
                  </svg>
                </a>
                <a href="https://wa.me/8618924949" target="_blank" class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:bg-[#25D366] hover:text-white hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-md">
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          @if (showNotifCta) {
            <div class="bg-gradient-to-r from-primary-green/10 to-emerald-500/10 border border-primary-green/30 dark:border-primary-green/20 rounded-2xl p-6 flex items-center gap-4 shadow-sm">
              <div class="w-12 h-12 rounded-full bg-primary-green/20 flex items-center justify-center text-primary-green shrink-0">
                <mat-icon>notifications_active</mat-icon>
              </div>
              <div class="flex-1">
                <p class="text-sm font-semibold text-slate-800 dark:text-white">Stay connected!</p>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Enable notifications to get a confirmation when I receive your message.</p>
              </div>
              <button (click)="enableNotifications()" class="px-4 py-2 rounded-lg bg-primary-green text-white text-xs font-semibold hover:bg-primary-green/90 transition-all shadow hover:shadow-md whitespace-nowrap">
                Enable
              </button>
            </div>
          }
...
export class ContactComponent implements AfterViewInit, OnInit {
  ...
  async enableNotifications() {
    const token = await this.firebaseService.requestPermissionAndGetToken();
    this.showNotifCta = false;
    if (token) {
      this.toastService.success('🔔 Notifications enabled! You\'ll get updates when I reply.');
      await this.firebaseService.sendTokenToBackend(this.http, token);
    } else {
      this.toastService.error('Could not enable notifications. Please allow in browser settings.');
    }
  }
  ...
  private playNotificationSound() {
    ...
    try {
      ...
      // Three ascending pleasant tones (like a success chime)
      ...
      this.http.post<{ success: boolean }>(
        ...
        next: async (response) => {
          ...
          if (response.success) {
            ...
            // 2. Play notification chime
            this.playNotificationSound();

            // 3. Show branded push notification to the person who filled the form
            const userName = this.contactForm.value.name || 'there';
            const subject = this.contactForm.value.subject || 'your inquiry';
            this.firebaseService.showBrandedNotification(
            ...
            // 4. Request notification permission if not already granted (first-time UX)
            if ('Notification' in window && Notification.permission === 'default') {
              const token = await this.firebaseService.requestPermissionAndGetToken();
              if (token) {
                await this.firebaseService.sendTokenToBackend(this.http, token);
